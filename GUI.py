...
def Convert(outText):
    outText.delete('1.0',END)
    outText.insert(END,'the json text will be: \n\n')
    path = pathentry.get()
    convert(s=path)
...
def Compress(outText):
    outText.delete('1.0',END)
    path = pathentry.get()
    getHuffmanCode(s=path)
    outText.insert(END,' "Encoded_file.txt is created succesfully\n')
...
Convert_to_JSON = Button(window, text='JSON',width=10,command=lambda :Convert(outText=outText))
...
compress_Button = Button(window, text='Compress',width=10,command=lambda :Compress(outText=outText))
